...
const commandHandlers: { [key: string]: () => Promise<void> } = {
	compost: async () => {
		...
		if (!duration.match(/^(\d+)([mhdwMy])$/) && duration != '0')
			throw new FecesError(
				`Invalid duration format (received '${duration}').`.red
			);
		const db = new Database<FileData>(join(homedir(), '.feces', 'index.json')),
			splitdate = ts - parseDuration(duration);
		...
		if (keyInYN('Are you sure you want to continue?')) {
			...
			log(
				`Composting complete (permanently deleted ${toCompost.length} files).`
			);
		} else log('Aborted.');
	},
	help: async () => {
		if (argv.length > 2)
			throw new FecesError(
				'Invalid number of subarguments (expected 0-1).'.red
			);
		const mainstring = '%s  %s',
			singlestring = '%s:\n%s';
		if (argv.length == 1) {
			log('Available commands:'.magenta);
			log(
				mainstring,
				'compost <duration>'.cyan,
				'Compost (permanently delete) all files older than {duration}'.blue
			);
			log(
				mainstring,
				'help              '.cyan,
				'Show this help message.'.blue
			);
			log(
				mainstring,
				'init              '.cyan,
				'Initialize the feces environment.'.blue
			);
			log(
				mainstring,
				'pie               '.cyan,
				'List all plopped files.'.blue
			);
			log(
				mainstring,
				'plop <file>       '.cyan,
				'Plop a file. Path may be relative or absolute.'.blue
			);
			log(
				mainstring,
				'plunge <file>     '.cyan,
				'Plunge a plopped file. {file} must be one of the unique file IDs as listed in the pie command'
					.blue
			);
		} else {
			switch (argv[1]) {
				case 'compost':
					log(
						singlestring,
						'compost <duration>'.cyan,
						"Compost (permanently delete) all trashed files older than {duration}. {duration} must be an integer followed directly by a lowercase alphabetic character (i.e. '10h', '4d', '2mo', etc.). {duration} may also be set to '0', which will compost all trashed files."
							.blue
					);
					break;
				case 'help':
				...
				case 'init':
					log(
						singlestring,
						'init'.cyan,
						'Initialize the feces environment. This will create a .feces directory in your home directory.'
							.blue
					);
					break;
				case 'pie':
					log(
						singlestring,
						'pie'.cyan,
						'List all plopped files. This will print a table of all plopped (trashed) files, including their unique IDs and original paths (before trashing).'
							.blue
					);
					break;
				case 'plop':
					log(
						singlestring,
						'plop <file>'.cyan,
						"Plop a file. Path of {file} may be relative or absolute. This will move the file to the .feces/files directory and create a record of the file in the '~/.feces/index.json', indexed by its unique ID."
							.blue
					);
					break;
				case 'plunge':
					log(
						singlestring,
						'plunge <file>'.cyan,
						"Plunge a plopped file. {file} must be one of the unique file IDs as listed in the pie command. This will move the file from the .feces/files directory back to its original path and remove the record of the file from '~/.feces/index.json'."
							.blue
					);
					break;
				default:
					throw new FecesError(
						`Invalid subargument '${argv[1]}': No such command`.red
					);
			}
		}
	},
	init: async () => {
		...
		try {
			...
			throw new FecesError(
				'Failed to initialize the feces environment; either already initialized, lacking permissions, or other problems.'.red
			);
		}
	},
	pie: async () => {
	...
	plunge: async () => {
		...
		const name = argv[1],
			db = new Database<FileData>(join(homedir(), '.feces', 'index.json')),
			fd = db.get(name);
		if (!fd) throw new FecesError('There is no such plopped file.'.red);
...
async function main(): Promise<void> {
	...
	if (commandHandlers[cmnd] === undefined) {
		throw new FecesError(
			`Command '${cmnd}' not found. Use 'feces help' to see available commands.`.yellow
		);
	} else commandHandlers[cmnd]().catch((err) => error(err.message));
}
