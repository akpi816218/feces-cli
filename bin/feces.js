...
const commandHandlers = {
    ...
    plop: async () => {
        ...
        const ts = Date.now(), name = `${ts}-${argv[1]}`, filepath = join(cwd(), argv[1]), db = new Jsoning(join(homedir(), '.feces', 'index.json')), newpath = join(homedir(), '.feces', 'files', name);
        try {
...
function main() {
    ...
    if (commandHandlers[cmnd] === undefined) {
        log(`Command '${cmnd}' not found. Use feces --help to see available commands.`);
    }
    else
        commandHandlers[cmnd]();
}
main();
